...
feature
	make(enemy_id: INTEGER)
		do
			...
			create symbole.make_from_string ("G")
	...
	set_alive (alive: BOOLEAN)
		do
			...
			if not is_alive then
				model_Access.m.score.add (create {SILVER}.make)
			end
		end
	...
	preemptive_action
		...
		do
			...
			if model_access.m.turn ~ "pass" then
				...
				output.append("%N    A "+ name + "(id:" + id.out + ") gains "+ "10" + " total health.")

			end
			if model_access.m.turn ~ "special" then
				current_health := current_health + 20
				total_health := total_health + 20
				output.append("%N    A "+ name + "(id:" + id.out + ") gains "+ "20" + " total health.")
