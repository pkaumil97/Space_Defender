...
feature
	make(enemy_id: INTEGER)
		do
			...
			create symbole.make_from_string ("P")
...
--		 	set_can_see_starfighter
	...
	in_vision_of_pylon(e: ENEMY) : 	BOOLEAN
		do
			if ((pos.e_r - e.pos.e_r).abs + (pos.e_c - e.pos.e_c).abs) <= vision  then
				Result := true
			end
		end
	set_on_board
	...
	set_alive (alive: BOOLEAN)
		do
			...
			if not is_alive then
				model_Access.m.score.add (create {PLATINUM}.make)
			end
		end
	health_regen
		do
	...
	action
		...
		do
			...
			if can_see_starfighter then
				...
				moving(2)
				if is_alive and on_board then
					across model_access.m.enemy_collection is e
					loop
						if e.is_alive and e.on_board and current.in_vision_of_pylon (e) then

							e.set_current_health (e.current_health + 10)
							model_access.m.print_state.set_enemy_action_section ("%N      The " + name +" heals " + e.name + "(id:" + e.id.out + ") at location [" +
														 model_access.m.print_state.rows[e.pos.e_r] +"," + e.pos.e_c.out +"] for 10 damage.")
						end
					end
				end
			end
											...
											elseif model_access.m.is_id_of_enemy_proj (retrived_id) then
												if attached model_access.m.enemy_proj_collection.at (retrived_id) as e_pro and then e_pro.is_alive and then e_pro.on_board then
													...
													output.append ("%N      The projectile collides with enemy projectile(id:"+ e_pro.id.out+
																				") at location [" + model_access.m.print_state.rows[e_p.pos.f_r] +"," + e_p.out + "], combining damage.")
